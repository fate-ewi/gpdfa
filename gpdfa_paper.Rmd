...
Dynamic factor analysis (DFA) represents a flexible approach for using state space models to capture latent processes in multivariate time series [@zuur2003b; @zuur2003c]. For some ecological datasets, using a random walk to model the latent DFA trends may be overly complex. Examples of cases where random walks may overfit trends may be when there are large gaps in time between observations, or data collected from systems with high signal to noise ratios. As alternatives to the conventional random walk, we illustrate how DFA trends may be modeled using Gaussian process models or B-splines. Both of these alternatives are flexible in that their smoothness may be specified a priori by the user, and compared via model selection. As the variability of latent trends is nearly always fixed in a conventional DFA for identifiability [@zuur2003b; @holmes2012b], adopting an alternative model of the trend does not limit inference or change the meaning of other parameters (e.g. loadings). 
