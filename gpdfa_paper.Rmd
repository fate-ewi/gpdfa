...
For our application of smooth dynamic factor models to the CalCOFI juvenile rockfish dataset, we found that the B-spline trend model with 6 knots had the highest ELPD values than alternative models (Table 1). Varying the number of knots for the B-spline or Gaussian process models had little effect on ELPD, and the B-spline models performed slighlty better than the Gaussian process models (Table 1). All smooth trend models had higher predictive accuracy compared to the conventional random walk DFA model. All three *Sebastes* species were estimated to have positive loadings on the estimated trend, with shortbelly rockfishes having the highest loading (`r round(post_means[3],2)`, 95% posterior interval = `r round(post_low[3],2)`-`r round(post_hi[3],2)`) followed by aurora (`r round(post_means[1],2)`, 95% posterior interval = `r round(post_low[1],2)`-`r round(post_hi[1],2)`) and bocaccio rockfishes with the weakest loadings (`r round(post_means[2],2)`, 95% posterior interval = `r round(post_low[2],2)`-`r round(post_hi[2],2)`). Though the model with 6 knots was found to have the highest predictive accuracy, the simplicity of this model was not able to capture the most recent patterns between 2015-2019, when juvenile rockfish densities experienced high densities and high variability (Fig 1).
...
We found that the when compared to conventional DFA with trends as random walks, modeling trends as smooth functions resulted in models with higher predictive ability for out of sample cross-validation. Both applications in our analysis highlighted that models with trends as B-splines received the highest support, and that these models had better predictive ability than Gaussian predictive process models. In evaluating 1-trend models of juvenile rockfish densities, we found that varying the number of knots had little effect on predictive accuracy, though models with $> 6$ knots were better able to capture recent variability in these time series. For our 2-trend models exploring the dynamics of groundfish catches, we also found that the smooth trend models outperformed the conventional random walk DFA models, and that modeling trends as B-splines resulted in higher predictive accuracy compared to the Gaussian process models. We used uniform knots for both types of smooth trend models, and these results would be expected to change slightly if the knot locations were adjusted. For models with missing data, or datasets with unevenly distributed replicate samples, it may be important to consider non-uniform knot vectors. 
...
tab = data.frame("Trend model"=c("Random walk", rep("B-spline",5), rep("Gaussian process",5)),
  ...
  "CalCOFI" = paste0(round(unlist(lapply(cv_calcofi,getElement,3)), 2), " (",round(unlist(lapply(cv_calcofi,getElement,4)),2),")"),
  "Landings" = paste0(round(unlist(lapply(cv_landings,getElement,3)), 2), " (",round(unlist(lapply(cv_landings,getElement,4)),2),")")
  )
knitr::kable(tab)
```
